...
.global _inputSource;
.var _inputSource[INS] = 0, 0;		// 0=analog, 1=noise, 2=sine
.global _inDelay, _outDelay;		// adjust delay (since DMA)
.var _inDelay[INS] = 1, 0;
.var _outDelay[OUTS] = 0, 1, 0, 1, 0, 1;
...
// attack time(in second)  =0.00001*pow(10, i/12), i=0,...60
.var _releaseTime[INS+OUTS];
// release time(in second) =0.00100*pow(10, i/12), i=0,...48

.global _compressorGain;
.var _compressorGain[INS] = 60.0, 60.0;	// based on compGain
.global _peqGain;
.var _peqGain[INS] = 0.0, 0.0;			// based on _PEQLevel
...
//	5:Notch Filter
//	0x81--GEQ
// all init as 0, 69, 24, 46, (PEQoff, 1000Hz, gain=1, Q=4.33)

// 输出滤波、参数均衡器预置
// HPFType, HPFFreq, LPFType, LPFFreq, 3x(PEQs)
// init as 0, 0, 0, 120(HPF/LPF type 0, HPfreq 20Hz, LPfreq 20000Hz)
// Equalizer as 0, 69, 24, 46, (PEQoff, 1000Hz, gain=1, Q=4.33)
...
.var _inp_coeffs[INS*4*IN_PEQS];			// PEQ coeffs of input(a2,a1,b2,b1...)
.var _outp_coeffs[OUTS*4*OUT_PEQS];			// PEQ coeffs of output(a2,a1,b2,b1...)
...
.var _SoftwareVersion[64] = 'Ver.0.61 Aug.08 2011            ',
							'                                ';
...
.var _programOutputStatus[6] = 0xC2, 0xC1, 0xC2, 0xC1, 0xC2, 0xC1;
...
.var _generatorSelect = 0;				// 0:pinknoise, 1:sine wave
...
.var _outputChannel[OUTS*26] =
		...
		0x10, 01, 0, 0, 0, 122, 0, 0x45, 0x18, 0x2e, 0, 0x45, 0x18, 0x2e,
		...
		0x02, 02, 0, 0, 0, 122, 0, 0x45, 0x18, 0x2e, 0, 0x45, 0x18, 0x2e,
