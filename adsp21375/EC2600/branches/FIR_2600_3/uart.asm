...
.extern _calculateFIR, _loadFromFlash, _transferAmplitude;
...
.var UartCommandTable[] =
	...
	'P', 6+2*N_OCTAVE, uart_P,	// (IDPCC, upload amplitude, Channel+256bytes
	'S', 6,	uart_S,				// (IDSFF,  Save Phase Data to FLASH
	'W', 6, uart_W,				// (IDWwd,  set window function
...
do_not_erase:
    ...
    dm(_HexData + 0) = m7;	if EQ jump (pc, uart_D1);
...
uart_W:									// Set Window Function
	r4  = 6;
	dm(CUART0TX) = r4;					// DMA counter
	call (pc, _FillTxBuffer);

	i12 = _uart_rxbuf + 4;
	call (pc, _GetNumberFromUart);		// Window Function.(R0)
