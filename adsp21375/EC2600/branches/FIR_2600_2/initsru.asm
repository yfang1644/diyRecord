...
_initSRU:
	...
	// ADC works in slave mode, PCG->DSP -> ADC bit clock and LR clock
	...
	SRU(PCG_CLKA_O, SPORT1_CLK_I);		// ADC bit clock
	SRU(PCG_FSA_O, SPORT1_FS_I);		// ADC LR clock

	SRU(PCG_CLKA_O, DAI_PB07_I);		// BCLK to ADC(AK5393)
	SRU(PCG_FSA_O, DAI_PB08_I);			// LR clock to ADC(AK5393)

	SRU(HIGH, PBEN07_I);				// set output to AK5393 BCLK
	SRU(HIGH, PBEN08_I);				// set output to AK5393 LRCLK
	SRU(DAI_PB05_O, SPORT1_DA_I);		// ADC(AK5393) SDATA input
//	SRU(LOW, DAI_PB05_I);
	SRU(LOW, PBEN05_I);

	SRU(PCG_CLKA_O, SPORT2_CLK_I);		// bit clock
	SRU(PCG_CLKA_O, SPORT3_CLK_I);		// bit clock 
	SRU(PCG_CLKA_O, DAI_PB13_I);		// bit clock to DAC1
	SRU(PCG_CLKA_O, DAI_PB15_I);		// bit clock to DAC2 and DAC3
	SRU(HIGH, PBEN13_I);				// BCLK output enable
	SRU(HIGH, PBEN15_I);				// BCLK output enable

	SRU(PCG_FSA_O, SPORT2_FS_I);		// LR clock
	SRU(PCG_FSA_O, SPORT3_FS_I);		// LR clock
	SRU(PCG_FSA_O, DAI_PB14_I);			// LR clock to DAC1
	SRU(PCG_FSA_O, DAI_PB16_I);			// LR clock to DAC2 and DAC3
	SRU(HIGH, PBEN14_I);				// LR clock output enable
	SRU(HIGH, PBEN16_I);				// LR clock output enable
