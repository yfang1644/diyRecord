...
.var UartCommandTable[] =
	...
	'h', 8, uart_hello,			// (IDhello
	'b', 6, uart_bye,			// (IDbye
	'C', 5, uart_C,				// (IDCE or (IDCD
	'K', 4, uart_K,				// (IDK
...
.var UartParameterTable[] =		// derived from (ID$
	...
	1, 10, P001,				// Source select(input or generator)
	2, 10, P002,				// Generator select(pink or sinewave)
	3, 10, P003,				// .pink noise level
...
_getLevelInformation:
	...
	lcntr = INS + OUTS, do (pc, _level1) until lce;
		...
		r8 = FIX f0;
		r8 = r8 - r3;					// >=100dB?
		if GE jump (pc, 3), r6 = r6 + 1;
		r8 = r8 + r9;
		if LE r8 = r8 - r8;				// negative, set to 000
		CCALL (_div10);
		r0 = r0 + r10, pm(i10, m14) = r6;
...
_setNewParameters:
	...
	lcntr = @UartCommandTable/3, do checkValid until lce;
		r10 = dm(i5, m6);
...
uart_hello:								// Communication Check, UART online
	...
	r0 = dm(_panelStatus);
	r0 = BSET r0 by 2;					// set LOCK bit
	jump (pc, uart_bye1)(DB);
	bit CLR IMASK IRQ1I;
...
uart_bye:								// UART offline
	...
	r4 = dm(_panelCommandTable + 0);	// panel status indicators
	r0 = dm(_panelStatus);
	r0 = BCLR r0 by 2;					// clear LOCK bit
	bit SET IMASK IRQ1I;
...
uart_C:									// Panel Lock('D') Unlock('E') setting
	...
	r3 = 0x45;							// 'E'
	r2 = 2;								// prepare for bit operation.
	bit CLR IMASK IRQ1I;				// disable switch interrupt
	bit SET ustat4 BIT_28;
	r4 = dm(_panelCommandTable + 0);	// panel status indicators
	r0 = dm(_panelStatus);
	comp (r8, r3);						// r8=_uart_rxbuf[4]
	if NE jump (pc, uart_C1), r0 = BSET r0 by r2;	// disable panel operation
	r0 = BCLR r0 by r2;					// enable panel operation
	bit SET IMASK IRQ1I;
uart_C1:
	dm(_panelStatus) = r0;
	r4 = BCLR r4 by 3;
	...
	r4 = 0;								// Save panel lock status into FLASH
	i2 = _remoteID;
...
uart_K:									// Panel Status Check(E or D)
	...
	r0 = dm(_panelStatus);
	r2 = 0x45;							// 'E'
	r0 = FEXT r0 by 2:1;				// if BIT2/panelstatus=0 then disabled
	r0 = r2 - r0;
...
P001:		//(ID$001VVV, Source select
	i4 = _uart_rxbuf + 7;
	r8 = 3;
	call _GetNumberFromUart;
	bit CLR ustat4 BIT_10;
	r4 = _samples + 1 - 0x00080000;		// ADC data into samples[0-1](usefull)
	r0 = pass r0;
	if EQ jump (pc, 3);
	bit SET ustat4 BIT_10;
	r4 = _samples + 3 - 0x00080000;		// ADC data into samples[2-3](trash)
	dm(TCB_Block_ADCx + 3) = r4;
	jump (pc, P007_L0 + 1);				// skip parameters init

P002:		//(ID$002VVV, pink or sine select
P003:		//(ID$003VVV, pink level
P004:		//(ID$004CVVV, Sine frequency
	r8 = 3;
	call _GetNumberFromUart;			// r0 = Frequency index(1~121)

	r2 = r0 - 1;
	if LT jump (pc, P007_L0 + 1);		// freq=0, do NOTHING
	dm(_sineFrequency) = r2;

	f2 = float r2;
	f1 = 0.0577622650466621;			// ln(2)/12
	f4 = f1 * f2, puts = r12;
	CCALL (_expf);
	f2 = 19.7 * MAXBUFFER / FS;			// freq=20*10^(i/12)
	f4 = f0 * f2, r12 = gets(1);
	r0 = FIX f4;
	...
	r12 = r12 - 1, alter(m6);
	if LT jump (pc, 4);
	jump (pc, P007_L0 + 1)(DB);			// skip parameters init
	m4 = r12;
	dm(m4, i2) = r0;
	jump (pc, P007_L0 + 1)(DB);			// skip parameters init;
	dm(i2, m6) = r0;
...
P005:		//(ID$005CVVV, sine level
	...
	dm(_sineLevel) = r0;
	r4 = 181;
	f2 = 2.30258509299404590109/40.0;
	r4 = r0 - r4, puts = r12;
	f4 = float r4;
	f4 = f2 * f4;
	CCALL (_expf);
...
P031:jump (pc, P007_L0 + 1);			// Compressor Side EQ Q
...
P020:		//(ID$020CVVV, PEQ Link
	...
	dm(_PEQLink) = r0;
	r0 = pass r0;
	if EQ jump (pc, P007_L0 + 1);		// OFF-link, do nothing
	...
	r12 = r12 - 1;
	if LT jump (pc, P007_L0 + 1);		// Link to 0? No!
